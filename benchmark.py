...
from minimal_ddp import DDPOverlap, DDPOverlapBucket
...
def setup_ddp(rank: int, world_size: int, backend: str = "nccl") -> str:
    ...
    os.environ.setdefault("MASTER_PORT", "29500")  # Changed from 12357 to avoid conflicts
    ...
    try:
        if torch.cuda.is_available() and backend == "nccl":
            ...
            dist.init_process_group(backend, rank=rank, world_size=world_size, device_id=local_rank)
        else:
...
def run(
    ...
    if overlap_individual:
        ...
        model = DDPOverlapBucket(model, bucket_size_mb=bucket_size_mb)
...
def _parse_args():
    ...
    parser.add_argument("--bucket-sizes-mb", type=float, nargs="+", default=[25])
